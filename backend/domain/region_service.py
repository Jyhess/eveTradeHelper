...
class RegionService:
    ...
    async def get_regions_with_details(self) -> list[RegionDetails]:
        region_ids = await self.repository.get_regions_list()

        async def fetch_region(region_id: int) -> RegionDetails | None:
            try:
    ...
    async def get_constellation_systems_with_details(
        ...
        async def fetch_system(system_id: int) -> SystemDetails | None:
            try:
    ...
    async def get_system_connections(self, system_id: int) -> list[SystemConnection]:
        system_data = await self.repository.get_system_details(system_id)
        stargate_ids = system_data.stargates or []

        source_constellation_id = system_data.constellation_id
        ...
        if source_constellation_id:
            ...
            source_region_id = source_constellation.region_id

        async def fetch_connection(stargate_id: int) -> SystemConnection | None:
            try:
                ...
                if isinstance(stargate_data, dict):
                    ...
                    destination_system_id = (
                        destination.get("system_id") if isinstance(destination, dict) else None
                    )
                else:
                ...
                if destination_system_id and destination_system_id != system_id:
                    ...
                    destination_constellation_id = destination_system.constellation_id
                    ...
                    if destination_constellation_id:
                        ...
                        destination_region_id = destination_constellation.region_id
                        destination_constellation_name = destination_constellation.name
                        ...
                        if destination_region_id and not same_region:
                            ...
                            destination_region_name = destination_region.name

                    return SystemConnection(
                        system_id=destination_system_id,
                        name=destination_system.name,
                        security_status=destination_system.security_status,
                        security_class=destination_system.security_class,
    ...
    async def get_systems_within_jumps(
        ...
        systems_map: dict[int, SystemMapSystem] = {}
        ...
        while queue:
            ...
            try:
                ...
                systems_map[current_system_id] = SystemMapSystem(
        ...
        return SystemsWithinJumpsResult(
            systems=list(systems_map.values()),
            connections=connections,
        )

    async def get_all_systems(self, name_filter: str | None = None) -> list[SystemSearchResult]:
        """
        ...
        """
        if name_filter:
            systems_dict = await self.region_data.find_system_by_name(name_filter)
        else:
            systems_dict = await self.region_data.get_all_systems()
    ...
    async def get_all_constellations(
    ...
    ) -> list[ConstellationSearchResult]:
        if name_filter:
            constellations_dict = await self.region_data.find_constellation_by_name(name_filter)
        else:
            constellations_dict = await self.region_data.get_all_constellations()
        ...
        constellations = [
            ...
            for c in constellations_dict
        ]

        return sorted(constellations, key=lambda x: x.name)

    async def search_systems(self, name_filter: str | None = None) -> list[SystemSearchResult]:
        all_systems = []
        region_ids = await self.repository.get_regions_list()
        ...
        for region_id in region_ids:
            try:
                ...
                for constellation in constellations:
                    ...
                    system_ids = constellation.systems
                    ...
                    for system_id in system_ids:
                        try:
                            ...
                            region_id_from_constellation = constellation_data.region_id
    ...
    async def search_constellations(
        self, name_filter: str | None = None
    ) -> list[ConstellationSearchResult]:
        all_constellations = []
        region_ids = await self.repository.get_regions_list()
